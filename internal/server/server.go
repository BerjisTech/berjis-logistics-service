...
func broadcastPosition(kind string, id string, lat, lng float64, ts time.Time) {
	for i := 0; i < len(wsSubs.list); i++ {
		sub := wsSubs.list[i]
		if kind == "vehicle" && sub.vehicleID != "" && sub.vehicleID != id {
			continue
		}
		if kind == "asset" && sub.assetID != "" && sub.assetID != id {
			continue
		}
...
func New(opts Options) *fiber.App {
	...
	registerStorefrontRoutes(app, opts)
	...
	app.Post("/v1/assets", func(c *fiber.Ctx) error {
		...
		if err := c.BodyParser(&in); err != nil || in.Kind == "" {
	...
	app.Post("/v1/assets/:id/assign", func(c *fiber.Ctx) error {
		...
		if err := c.BodyParser(&in); err != nil || in.VehicleID == "" {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false})
		}
	...
	app.Post("/v1/ingest/telemetry", func(c *fiber.Ctx) error {
		...
		if len(fixes) == 0 {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false, "message": "no positions"})
		}
		for _, f := range fixes {
			ts := time.Now().UTC()
			if f.Ts != nil {
	...
	// Shipments list
	app.Get("/v1/shipments", func(c *fiber.Ctx) error {
		if opts.DB == nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false})
		}
		uid := auth.UserID(c)
		...
		limit := clampInt(c.QueryInt("limit", 100), 1, 200)
		...
		type shipmentRow struct {
			...
			TrackingEnabled bool      `db:"tracking_enabled" json:"trackingEnabled"`
			OrderID         *string   `db:"order_id" json:"orderId,omitempty"`
			VehicleID       *string   `db:"vehicle_id" json:"vehicleId,omitempty"`
			VehiclePlate    *string   `db:"vehicle_plate" json:"vehiclePlate,omitempty"`
			DriverID        *string   `db:"driver_id" json:"driverId,omitempty"`
			DriverName      *string   `db:"driver_name" json:"driverName,omitempty"`
			Role            string    `db:"role" json:"role"`
			CreatedAt       time.Time `db:"created_at" json:"createdAt"`
			UpdatedAt       time.Time `db:"updated_at" json:"updatedAt"`
			NextStopSeq     *int      `db:"next_stop_seq" json:"nextStopSeq,omitempty"`
			NextStopAddress *string   `db:"next_stop_address" json:"nextStopAddress,omitempty"`
			StopCount       int       `db:"stop_count" json:"stopCount"`
		}
		rows := []shipmentRow{}
		if err := opts.DB.Select(&rows, `
			SELECT
				...
				CASE
					...
					WHEN dr.user_id = $1 THEN 'driver'
					ELSE 'observer'
				END AS role,
				d.created_at,
				d.updated_at,
				next_stop.seq AS next_stop_seq,
				next_stop.address AS next_stop_address,
				COALESCE(stop_stats.stop_count, 0) AS stop_count
			FROM deliveries d
			LEFT JOIN vehicles v ON v.id = d.vehicle_id
			LEFT JOIN drivers dr ON dr.id = d.driver_id
			LEFT JOIN LATERAL (
				SELECT rs.seq, rs.address
				FROM route_stops rs
				WHERE rs.delivery_id = d.id
				  AND (rs.status IS NULL OR rs.status NOT IN ('completed','departed'))
				ORDER BY rs.seq ASC
				LIMIT 1
			) next_stop ON TRUE
			LEFT JOIN LATERAL (
			...
			WHERE v.owner_user_id = $1 OR dr.user_id = $1
			ORDER BY d.updated_at DESC, d.created_at DESC
			LIMIT $2 OFFSET $3`, uid, limit, offset); err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false})
		}
		return c.JSON(fiber.Map{"success": true, "data": rows})
	})
	...
	app.Post("/v1/shipments/:id/route", func(c *fiber.Ctx) error {
		...
		if err := c.BodyParser(&in); err != nil || strings.TrimSpace(in.Address) == "" {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false})
		}
		var next int
	...
	app.Post("/v1/deliveries/:id/complete", func(c *fiber.Ctx) error {
		...
		if !allowed {
			_ = opts.DB.Get(&allowed, `SELECT EXISTS (SELECT 1 FROM deliveries d JOIN drivers dr ON dr.id=d.driver_id WHERE d.id=$1 AND dr.user_id=$2)`, did, uid)
		}
		if !allowed {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"success": false})
		}
	...
	app.Put("/v1/transport/jobs/:id/assign", func(c *fiber.Ctx) error {
		...
		n, _ := res.RowsAffected()
		if n == 0 {
			return c.Status(fiber.StatusConflict).JSON(fiber.Map{"success": false, "message": "job not available"})
		}
	...
	app.Post("/v1/transport/jobs/:id/tracking", func(c *fiber.Ctx) error {
		...
		if job.DriverID == nil || *job.DriverID != did {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"success": false})
		}
		if job.VehicleID == nil {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false, "message": "vehicle not set"})
		}
		var in struct {
			Lat float64 `json:"lat"`
			Lng float64 `json:"lng"`
	...
	app.Get("/v1/drivers/me", func(c *fiber.Ctx) error {
		...
		var d struct {
			...
			LicenseNo   *string        `db:"license_no" json:"licenseNo"`
	...
	app.Post("/v1/deliveries/:id/public-tracking", func(c *fiber.Ctx) error {
		...
		// Fetch current code
		var cur struct {
			Code    *string `db:"tracking_code"`
			Enabled bool    `db:"tracking_enabled"`
		}
	...
	app.Post("/v1/route-stops/:id/coords", func(c *fiber.Ctx) error {
		...
		if err := c.BodyParser(&in); err != nil || in.Lat == nil || in.Lng == nil {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false})
		}
		var perm struct {
			DeliveryID     string  `db:"delivery_id"`
			VehicleOwnerID *string `db:"vehicle_owner_id"`
		...
		allowed := false
		if perm.VehicleOwnerID != nil && *perm.VehicleOwnerID == uid {
			allowed = true
		}
		if perm.DriverUserID != nil && *perm.DriverUserID == uid {
			allowed = true
		}
		if !allowed {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"success": false})
