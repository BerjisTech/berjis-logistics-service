...
func New(opts Options) *fiber.App {
    ...
    app.Get("/v1/public/products/:id", func(c *fiber.Ctx) error {
        ...
        // Stores: create (enforce non-paying users max 1 store unless platform.admin/admin/paid role)
    app.Post("/v1/stores", func(c *fiber.Ctx) error {
        uid := auth.UserID(c)
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        var in struct{ Name string `json:"name"`; Slug *string `json:"slug"`; Description *string `json:"description"` }
        if err := c.BodyParser(&in); err != nil || strings.TrimSpace(in.Name)=="" { return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false, "message": "name required"}) }
        // Check roles
        var privileged bool
        ...
        slug := ""
        if in.Slug != nil && strings.TrimSpace(*in.Slug) != "" { slug = *in.Slug } else { slug = strings.ToLower(strings.ReplaceAll(strings.TrimSpace(in.Name), " ", "-")) }
        // Validate slug
        if slug == "" { return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false, "message": "invalid slug"}) }
        // Ensure uniqueness (case-insensitive)
        var exists bool
        _ = opts.DB.Get(&exists, `SELECT EXISTS (SELECT 1 FROM stores WHERE LOWER(slug)=LOWER($1))`, slug)
        if exists { return c.Status(fiber.StatusConflict).JSON(fiber.Map{"success": false, "message": "slug taken"}) }
        var out struct{ ID string `db:"id" json:"id"`; Name string `db:"name" json:"name"`; Slug string `db:"slug" json:"slug"` }
        if err := opts.DB.Get(&out, `INSERT INTO stores (owner_user_id, name, slug, description) VALUES ($1,$2,$3,$4) RETURNING id, name, slug`, uid, in.Name, slug, in.Description); err != nil {
    ...
    app.Get("/v1/stores", func(c *fiber.Ctx) error {
        uid := auth.UserID(c)
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        rows := []struct{ ID string `db:"id" json:"id"`; Name string `db:"name" json:"name"`; Slug string `db:"slug" json:"slug"`; Description *string `db:"description" json:"description"` }{}
    ...
    app.Post("/v1/products", func(c *fiber.Ctx) error {
        uid := auth.UserID(c)
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        var in struct{ SKU *string `json:"sku"`; Name string `json:"name"`; Description *string `json:"description"`; Price *float64 `json:"price"`; Currency *string `json:"currency"` }
        if err := c.BodyParser(&in); err != nil || strings.TrimSpace(in.Name)=="" { return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false}) }
        var out struct{ ID string `db:"id" json:"id"`; Name string `db:"name" json:"name"`; SKU *string `db:"sku" json:"sku"` }
        if err := opts.DB.Get(&out, `INSERT INTO products (owner_user_id, sku, name, description, price, currency) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, name, sku`, uid, in.SKU, in.Name, in.Description, in.Price, in.Currency); err != nil {
            return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false})
        }
        return c.Status(fiber.StatusCreated).JSON(fiber.Map{"success": true, "data": out})
    })
    ...
    app.Get("/v1/products", func(c *fiber.Ctx) error {
        uid := auth.UserID(c)
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        rows := []struct{ ID string `db:"id" json:"id"`; SKU *string `db:"sku" json:"sku"`; Name string `db:"name" json:"name"`; Price *float64 `db:"price" json:"price"`; Currency *string `db:"currency" json:"currency"` }{}
    ...
    app.Post("/v1/stores/:id/products", func(c *fiber.Ctx) error {
        sid := c.Params("id"); uid := auth.UserID(c)
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        // Ensure store belongs to user (or admin)
        var allowed bool
        _ = opts.DB.Get(&allowed, `SELECT EXISTS (SELECT 1 FROM stores WHERE id=$1 AND owner_user_id=$2)`, sid, uid)
        if !allowed { _ = opts.DB.Get(&allowed, `SELECT EXISTS (SELECT 1 FROM user_roles WHERE user_id=$1 AND role IN ('platform.admin','admin'))`, uid) }
        if !allowed { return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"success": false}) }
        var in struct{ ProductID string `json:"productId"`; PriceOverride *float64 `json:"priceOverride"` }
        if err := c.BodyParser(&in); err != nil || strings.TrimSpace(in.ProductID)=="" { return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false}) }
        _, err := opts.DB.Exec(`INSERT INTO store_products (store_id, product_id, price_override) VALUES ($1,$2,$3) ON CONFLICT (store_id, product_id) DO UPDATE SET price_override=EXCLUDED.price_override`, sid, in.ProductID, in.PriceOverride)
    ...
    app.Delete("/v1/stores/:id/products/:pid", func(c *fiber.Ctx) error {
        sid := c.Params("id"); pid := c.Params("pid"); uid := auth.UserID(c)
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false})
    ...
    app.Get("/v1/stores/:id/products", func(c *fiber.Ctx) error {
        ...
        if !allowed { _ = opts.DB.Get(&allowed, `SELECT EXISTS (SELECT 1 FROM user_roles WHERE user_id=$1 AND role IN ('platform.admin','admin'))`, uid) }
        if !allowed { return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"success": false}) }
        rows := []struct{ ID string `db:"id" json:"id"`; Name string `db:"name" json:"name"`; SKU *string `db:"sku" json:"sku"`; Price *float64 `db:"price" json:"price"`; Currency *string `db:"currency" json:"currency"`; PriceOverride *float64 `db:"price_override" json:"priceOverride"` }{}
        if err := opts.DB.Select(&rows, `SELECT p.id, p.name, p.sku, p.price, p.currency, sp.price_override FROM store_products sp JOIN products p ON p.id = sp.product_id WHERE sp.store_id=$1 ORDER BY p.name`, sid); err != nil {
            return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false})
        }
        return c.JSON(fiber.Map{"success": true, "data": rows})
    })
    ...
    app.Get("/v1/admin/users/:id/roles", func(c *fiber.Ctx) error {
        uid := auth.UserID(c); target := c.Params("id")
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        var isAdmin bool
        ...
        roles := []struct{ Role string `db:"role" json:"role"` }{}
    ...
    app.Post("/v1/admin/users/:id/roles", func(c *fiber.Ctx) error {
        ...
        _ = opts.DB.Get(&isAdmin, `SELECT EXISTS (SELECT 1 FROM user_roles WHERE user_id=$1 AND role='platform.admin')`, uid)
        if !isAdmin { return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"success": false}) }
        var in struct{ Role string `json:"role"` }
        if err := c.BodyParser(&in); err != nil || strings.TrimSpace(in.Role)=="" { return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false}) }
    ...
    app.Get("/v1/public/store/:slug", func(c *fiber.Ctx) error {
        slug := c.Params("slug")
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        var store struct{ ID string `db:"id" json:"id"`; Name string `db:"name" json:"name"`; Slug string `db:"slug" json:"slug"`; Description *string `db:"description" json:"description"` }
        ...
        products := []struct{ ID string `db:"id" json:"id"`; Name string `db:"name" json:"name"`; SKU *string `db:"sku" json:"sku"`; Price *float64 `db:"price" json:"price"`; Currency *string `db:"currency" json:"currency"`; PriceOverride *float64 `db:"price_override" json:"priceOverride"` }{}
